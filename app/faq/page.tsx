    ...
    <main style={{ backgroundColor: '#0a0a0a', color: '#d0d0d0', fontFamily: 'sans-serif' }}>
        ...
        <div style={{ maxWidth: '760px', margin: '0 auto', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {faqs.map((faq, i) => (
            <ScrollReveal key={i} direction="up">
              <div
                style={{ background: '#1a1a1a', border: `1px solid ${open === i ? '#4a7c59' : '#2a2a2a'}`, borderRadius: '12px', overflow: 'hidden', transition: 'border-color 0.3s' }}
              >
                <button
                ...
                >
                  <span style={{ color: '#C0C0C0', fontWeight: 700, fontSize: '1rem', lineHeight: 1.5 }}>{faq.q}</span>
                ...
                {open === i && (
                  <div style={{ padding: '0 2rem 1.5rem', borderTop: '1px solid #2a2a2a' }}>
                    <p style={{ color: '#888', lineHeight: 1.8, fontSize: '0.95rem', marginTop: '1.25rem' }}>{faq.a}</p>
                  </div>
                )}
              </div>
            </ScrollReveal>
      ...
      <ScrollReveal>
        <section style={{ padding: '100px 20px', textAlign: 'center', background: 'linear-gradient(180deg, #0f1a0f 0%, #0a0a0a 100%)', borderTop: '1px solid #1e2e1e' }}>
          ...
          <p style={{ color: '#888', marginBottom: '2.5rem', fontSize: '1.1rem' }}>Just call, text, or shoot us an email.</p>
